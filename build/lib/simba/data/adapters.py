...
class SimbaJSONDataset(Dataset):
    ...
    def __init__(
        ...
        if self.dups_index is not None:
            keys &= set(self.dups_index)  # ensures neighbors exist for Tang
        self.items = sorted(keys)
        
        if len(self.items) == 0:
            msg = []
            msg.append("No overlapping base keys across ys/coords" + (" and dups" if self.dups_index else "") + ".")
            msg.append(f"ys={len(self.ys)} coords={len(self.coords)} dups={'None' if self.dups_index is None else len(self.dups_index)}")
            msg.append(f"example ys key: {next(iter(self.ys)) if self.ys else 'EMPTY'}")
            msg.append(f"example coords key: {next(iter(self.coords)) if self.coords else 'EMPTY'}")
            if self.dups_raw:
                msg.append(f"dup raw example key: {next(iter(self.dups_raw))}")
            msg.append("Hint: we now match by basename and keep full paths; ensure basenames correspond (e.g., 107022.tiff ↔ 107022_1.tiff).")
            raise ValueError("\n".join(msg))
